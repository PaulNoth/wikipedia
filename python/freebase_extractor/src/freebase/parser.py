...
def filter_triples(triples, config):
    """
    Filters a list of localized triples created by the
    parse_and_localize function according to a configuration
    dict.
    """
...
def query_result_to_entity_info(result_list, config):
    ...
    from multiple languages are replaced with a single entry for
    each (predicate ID, object data) pair with the language code
    set to 'link'.
    """
